...
function loadFolders() {
    // Отправляем запрос для получения списка папок
    performLfmRequest('folders', {}, 'html')
        .done(function (data) {
            // Загружаем полученные данные в элемент с id 'tree'
            $('#tree').html(data);

            // Загружаем элементы (например, файлы)
            loadItems();

            // Закрываем все модальные окна с классом 'show'
            document.querySelectorAll('.show').forEach(item => {
                console.log(1);
                // Преобразуем item в jQuery-объект и закрываем модальное окно
                $(item).modal('hide');
            });
        });
}
...
function preview(items) {
    ...
    items.forEach(function (item, index) {
        ...
        if (item.url.endsWith('.pdf')) {
            ...
            carouselItem.find('.carousel-image').html(`<embed src="${item.url}" width="992px" height="2100px" />`);  // Добавляем canvas в элемент карусели
